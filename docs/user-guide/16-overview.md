...
| Actual monthly spend (Synapse) | sum across `cost.monthly_totals` | uses the average month per workspace |
...
- **Actual $/mo (Synapse)** — average monthly cost across the cost
  window. Empty if the cost module didn't run.
