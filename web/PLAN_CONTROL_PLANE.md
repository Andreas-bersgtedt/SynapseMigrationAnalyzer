...
3. **Re-use the existing analyzer.** Every endpoint is a thin adapter
   over a function already used by [`cli.py`](../src/synapse_migration_analyzer/cli.py).
...
| `POST` | `/api/config/validate` | `AppConfigUpdate` | `{ "ok": bool, "checks": [{"name": "odbc_driver", "ok": true, "detail": "ODBC Driver 18 found"}, ...] }`. Wraps the existing [`doctor.py`](../src/synapse_migration_analyzer/doctor.py) checks plus a SQL reachability probe per dedicated pool. Never calls Azure if `--offline` is set. |
...
```python
# src/synapse_migration_analyzer/web/jobs.py
...
```python
# src/synapse_migration_analyzer/web/api/config.py
