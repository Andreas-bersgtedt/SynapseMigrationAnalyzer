# Contributing to Synapse Migration Analyzer
...
- [ ] Type-check is clean for changed files (`mypy src/synapse_migration_analyzer/...`).
...
Each module under `src/synapse_migration_analyzer/modules/<name>/` exposes:
...
1. Wire it into `src/synapse_migration_analyzer/cli.py` (own `analyze-...`
   command + the appropriate `[N/M]` step in `analyze-all`).
2. Add an entry to `_MODULES` in
   `src/synapse_migration_analyzer/reporting/index_report.py`.
